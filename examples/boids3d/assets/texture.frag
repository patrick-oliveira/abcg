...
vec4 BlinnPhong(vec3 N, vec3 L, vec3 V, vec2 texCoord, float attenuation) {
  N = normalize(N);
  L = normalize(L);
  ...
  vec4 diffuseColor = map_Kd * Kd * Id * lambertian * attenuation;
  vec4 specularColor = Ks * Is * specular * attenuation;
...
void main() {
  ...
  if (mappingMode == 0) {
    ...
    vec4 color1 = BlinnPhong(fragN, fragL, fragV, texCoord1, attenuation);

    // Sample with y planar mapping
    vec2 texCoord2 = PlanarMappingY(fragPObj);
    vec4 color2 = BlinnPhong(fragN, fragL, fragV, texCoord2, attenuation);

    // Sample with z planar mapping
    vec2 texCoord3 = PlanarMappingZ(fragPObj);
    vec4 color3 = BlinnPhong(fragN, fragL, fragV, texCoord3, attenuation);
    ...
    color = BlinnPhong(fragN, fragL, fragV, texCoord, attenuation);
  }
