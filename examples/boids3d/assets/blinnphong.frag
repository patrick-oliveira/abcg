...
vec4 BlinnPhong(vec3 N, vec3 L, vec3 V, float attenuation) {
  N = normalize(N);
  L = normalize(L);
  ...
  vec4 diffuseColor = Kd * Id * lambertian * attenuation;
  vec4 specularColor = Ks * Is * specular * attenuation;
  vec4 ambientColor = Ka * Ia * attenuation;
...
void main() {
  ...
  vec4 color = BlinnPhong(fragN, fragL, fragV, attenuation);
